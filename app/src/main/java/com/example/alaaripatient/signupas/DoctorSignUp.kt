...
import com.example.alaaripatient.databinding.ActivityDoctorSignUpBinding
...
class DoctorSignUp : AppCompatActivity() {
    ...
    private fun vaidation() {
        ...
        binding.experienceEd.addTextChangedListener(object :TextWatcher{
            override fun beforeTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
            ...
            override fun afterTextChanged(p0: Editable?) {
                 if (p0!!.length==1 || p0!!.length==2 ){
                    binding.expV.visibility=View.VISIBLE
                    binding.expV.setImageResource(R.drawable.done)
                }else if (p0!!.length>2){
                    binding.expV.visibility=View.VISIBLE
                    binding.expV.setImageResource(R.drawable.wrong)
                }else if (p0!!.length==0){
                    binding.expV.visibility=View.INVISIBLE
                }
            }

        })
        binding.titleEd.addTextChangedListener(object :TextWatcher{
            override fun beforeTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
            ...
            override fun afterTextChanged(p0: Editable?) {
                if (p0?.length==0){
                    binding.titleV.visibility=View.INVISIBLE
                }else if (p0!!.length>12){
                    binding.titleV.visibility=View.VISIBLE
                    binding.titleV.setImageResource(R.drawable.done)
                }else if (p0?.length!! >20){
                    binding.titleV.visibility=View.VISIBLE
                    binding.titleV.setImageResource(R.drawable.wrong)
                }else if (p0!!.length<12 && p0!!.length>20 && p0!!.length>1){
                    binding.titleV.visibility=View.VISIBLE
                    binding.titleV.setImageResource(R.drawable.wrong)
                }
            }
    ...
    private fun selectCountrySpinner() {
        ...
        binding.signInBtn.setOnClickListener {
                ...
                val experince=binding.experienceEd.text.toString()
                ...
                lifecycleScope.launch{
                    val modal=SignupResponse(age,"f","$area","$city","$counrty",
                        ...
                        ,"$password","$language",
                        "$street","$title","$experince","$usertype"
                    )
                    val call= RetrofitObject.api.userregister(modal)
                    if (call.body()!=null){
                        SessionManager.saveAuthToken(this@DoctorSignUp,call!!.body()!!.data.data.token.accessToken)
                        if ( call.body()!!.isSuccess){
                            startActivity(Intent(this@DoctorSignUp,BottomActivity::class.java))
                        }
                    }
                }
